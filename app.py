...
@app.get("/account/change-password", include_in_schema=False)
async def member(request: Request):
	return FileResponse("./static/change_pass.html", media_type="text/html")
@app.get("/account/edit-profile", include_in_schema=False)
async def member(request: Request):
...
@app.delete("/api/user/delete-profile")
async def update_password(request: Request, delete_profile: deleteProfile, response: Response):
    #檢查是否有token
    token = request.headers.get('Authorization')
    if not token:
