...
headers = {
    ...
    "pragma": "no-cache",
    "priority": "u=1, i",
    "referer": "https://www.live-light.com/shop/category/office-9?",
...
# Category to parse
category = "Office"
category_url = f"https://www.live-light.com/shop/category/office-9"

page = 1  # Start from page 2
                ...
                try:
                    ...
                    for product in all_data:
                        if product['id'] == product_id:
                            # Check if the product already has the "office" category
                            if 'category' in product and "office" in product['category']:
                                print(f"Product {product_id} already has category 'office'. Stopping parsing.")
                                stop_parsing = True
                                break

                            # Add the "office" category to the product if not already present
                            if 'category' in product:
                                if "office" not in product['category']:
                                    product['category'].append("office")
                            else:
                                product['category'] = ["office"]

                            print(f"Added 'office' category to product {product_id}")
