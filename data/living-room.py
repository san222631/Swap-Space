...
headers = {
    ...
    "pragma": "no-cache",
    "priority": "u=1, i",
    "referer": "https://www.live-light.com/shop/category/living-room-8?",
...
# Category to parse
category = "Woonkamer"
category_url = f"https://www.live-light.com/shop/category/living-room-8"

page = 1  # Start from page 2, as requested
                ...
                try:
                    ...
                    for product in all_data:
                        if product['id'] == product_id:
                            # Check if the product already has the "living" category
                            if 'category' in product and "living" in product['category']:
                                print(f"Product {product_id} already has category 'living'. Stopping parsing.")
                                stop_parsing = True
                                break

                            # Add the "living" category to the product if not already present
                            if 'category' in product:
                                if "living" not in product['category']:
                                    product['category'].append("living")
                            else:
                                product['category'] = ["living"]

                            print(f"Added 'living' category to product {product_id}")
