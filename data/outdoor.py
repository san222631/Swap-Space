...
headers = {
    ...
    "pragma": "no-cache",
    "priority": "u=1, i",
    "referer": "https://www.live-light.com/shop/category/outdoor-173?",
...
# Category to parse
category = "Outdoor"
category_url = f"https://www.live-light.com/shop/category/outdoor-173"
                ...
                try:
                    ...
                    for product in all_data:
                        if product['id'] == product_id:
                            # Check if the product already has the "outdoor" category
                            if 'category' in product and "outdoor" in product['category']:
                                print(f"Product {product_id} already has category 'outdoor'. Stopping parsing.")
                                stop_parsing = True
                                break

                            # Add the "outdoor" category to the product if not already present
                            if 'category' in product:
                                if "outdoor" not in product['category']:
                                    product['category'].append("outdoor")
                            else:
                                product['category'] = ["outdoor"]

                            print(f"Added 'outdoor' category to product {product_id}")
