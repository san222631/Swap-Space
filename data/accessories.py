...
headers = {
    ...
    "pragma": "no-cache",
    "priority": "u=1, i",
    "referer": "https://www.live-light.com/shop/category/accessories-11?",
...
category_url = f"https://www.live-light.com/shop/category/accessories-11"
                ...
                try:
                    ...
                    for product in all_data:
                        if product['id'] == product_id:
                            # Check if the product already has the "accessories" category
                            if 'category' in product and "accessories" in product['category']:
                                print(f"Product {product_id} already has category 'accessories'. Stopping parsing.")
                                stop_parsing = True
                                break

                            # Add the "accessories" category to the product if not already present
                            if 'category' in product:
                                if "accessories" not in product['category']:
                                    product['category'].append("accessories")
                            else:
                                product['category'] = ["accessories"]

                            print(f"Added 'accessories' category to product {product_id}")
